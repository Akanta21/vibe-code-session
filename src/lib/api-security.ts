...
export class ApiSecurity {
  ...
  // Apply rate limiting
  static async applyRateLimit(request: NextRequest, type: keyof typeof rateLimiters) {
      ...
      return NextResponse.json(
        ...
        { 
          ...
          headers: {
            ...
            'X-RateLimit-Limit': SECURITY_CONFIG.rateLimits[type].maxRequests.toString(),
  ...
  // Validate API key
  static validateApiKey(request: NextRequest, requiredLevel: 'admin' | 'internal') {
    const apiKey = request.headers.get('x-api-key');
    const expectedKey = SECURITY_CONFIG.apiKeys[requiredLevel];
    
    if (!expectedKey) {
      console.warn(`API key not configured for level: ${requiredLevel}`);
      return false;
    }
  ...
  static validateInput(data: any, schema: Record<string, any>) {
    ...
    for (const [field, rules] of Object.entries(schema)) {
      ...
      if ((value === undefined || value === null || (typeof value === 'string' && !value.trim())) && !rules.required) {
        continue;
      }
  ...
  static async validateRequest(
    ...
    const { rateLimitType, requireApiKey, validateOrigin = true, inputSchema } = options;

    // Rate limiting
    if (rateLimitType) {
      const rateLimitResult = await this.applyRateLimit(request, rateLimitType);
      if (rateLimitResult) {
        this.logSecurityEvent('RATE_LIMIT_EXCEEDED', request, { type: rateLimitType });
    ...
    if (requireApiKey) {
      if (!this.validateApiKey(request, requireApiKey)) {
        this.logSecurityEvent('INVALID_API_KEY', request, { requiredLevel: requireApiKey });
        return NextResponse.json(
    ...
    if (inputSchema && (request.method === 'POST' || request.method === 'PUT')) {
      try {
        const body = await request.json();
        const validation = this.validateInput(body, inputSchema);
        
        if (!validation.isValid) {
          this.logSecurityEvent('INVALID_INPUT', request, { errors: validation.errors });
          return NextResponse.json(
        ...
        this.logSecurityEvent('MALFORMED_JSON', request, { error: (error as Error).message });
        return NextResponse.json(
          { error: 'invalid_json', message: 'Malformed JSON in request body' },
          { status: 400 }
        );
      }
    }
...
export async function withSecurity(
  ...
  try {
    const securityResult = await ApiSecurity.validateRequest(request, options);
    ...
    ApiSecurity.logSecurityEvent('HANDLER_ERROR', request, { error: (error as Error).message });
